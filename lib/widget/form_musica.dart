...
class _FormMusicaState extends State<FormMusica> {
  ...
  DAOCategoriaMusica daoCategoriaMusica = DAOCategoriaMusica();
  DAOArtistaBanda daoArtistaBanda = DAOArtistaBanda();
  ...
  List<DTOCategoriaMusica> _categoriasMock = [];
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text('Categorias (${dto.categorias.length}):',
                ...
                if (dto.linksVideoAula.isNotEmpty) ...[
                  const SizedBox(height: 8),
                  Text('Links de Vídeo Aula (${dto.linksVideoAula.length}):',
                      style: const TextStyle(fontWeight: FontWeight.bold)),
  ...
  void _loadData() async {
    final _categoriaBanco = await daoCategoriaMusica.buscarTodos();
    print(_categoriaBanco);
    setState(() {
      _categoriasMock = _categoriaBanco;
    });
  }
              ...
              FutureBuilder(
                  ...
                  builder: (context, future) {
                    if (future.connectionState == ConnectionState.waiting) {
                      ...
                      return Text('Erro ao carregar salas');
                    } else if (!future.hasData || future.data!.isEmpty) {
                      return Text('Nenhuma sala encontrada');
                    }

                    final _artista = future.data!;
                    return CampoOpcoes<DTOArtistaBanda>(
                      opcoes: _artista,
